void XBEE3_serial2_setting(){
  while(at_num_serial2 < 14){
    XBEE3_serial2_Receiving_Data();
    if(at_get_serial2 == true && at_num_serial2 < 14){
      Serial2.write(xbee3_ATcommand_DB2[at_num_serial2],len[at_num_serial2]);
      at_get_serial2 = false;
      //Serial.println("send");
    }
  }
}
    ...
    if (received_data_confirm[0] == 0x7E) {                        //api1 모드 일때 첫 데이터 0x7E인지 확인
      ...
      switch(received_data[0]){                                                                       //패킷 종류 구분 switch
        case 0x90: {                                                                                          //Receieve패킷 일때
          ...
          serial2_Mode_Change(received_SH , received_SL , received_MY , received_ASCII, ASCII_LENGTH);
          return 0;   break;
        }
        case 0x8B: {                                                                                                            //transmit_status패킷 일때
          status_chk_serial2 = true;
...
//            for (int i = 0; i < 4; i++)
//              my_address_serial2[i] = received_data[i + 5];
//          }   //my_addr 저장용
...
//****************************************Mode_change()****************************************************************
int serial2_Mode_Change(byte SH[4], byte SL[4], byte MY[2], byte DATA[], int DATA_LENGTH){        // Receive packet일때 실행되는 함수
  String data_string = "";
  for(int i = 0 ; i < DATA_LENGTH; i++)
    data_string += String((char)DATA[i]);
  for(int i = G2_P1; i <= G2_P8; i++)
    if(SL[2] == G2[i].mac[6] && SL[3] == G2[i].mac[7]){
      G2[i].state = true;
      Serial.println(G2[i].name + ": \b");
      Serial.println(data_string);
    }
}
//****************************************XBEE3_serial2_Transmit_Request()****************************************************************
void XBEE3_serial2_Transmit_Request_ASCII(byte adr[8], String str) {
  ...
  Serial2.write(transmit_data, sizeof(transmit_data));  // Xbee3로 전송
...
//  }
//  Serial.println();
  while(status_chk_serial2 != true)                            //transmit status를 받기 전까지 반복
    XBEE3_serial2_Receiving_Data();
  status_chk_serial2 = false;
}
//****************************************XBEE3_serial2_Transmit_Request()****************************************************************
