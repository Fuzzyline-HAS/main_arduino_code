void XBEE3_serial1_setting(){
  while(at_num_serial1 < 14){
    ...
    if(at_get_serial1 == true && at_num_serial1 < 14){
      Serial1.write(xbee3_ATcommand_DB[at_num_serial1],len[at_num_serial1]);
...
//  Serial.println();
  while(status_chk_serial1 != true)                            //transmit status를 받기 전까지 반복
    transmit_status_result = XBEE3_serial1_Receiving_Data();
  status_chk_serial1 = false;
  return transmit_status_result;
}

//****************************************XBEE3_Transmit_Request_HEX()****************************************************************
byte XBEE3_Transmit_Request_HEX(byte adr[8], byte hex[], int hex_length) {
    ...
    //패킷 세팅
    transmit_data[2] = data_length - 3 - 1;                                                         //LENGTH: 총길이 - startdelimiter-legnth-checksum
    for (int i = 5; i <= 12; i++)                   transmit_data[i] = adr[i - 5];      //MAC_ADDR: 전역 배열에서 몇번쨰인지 불러와서 매핑
    for (int i = 17; i < data_length - 1; i++)      transmit_data[i] = hex[i - 17];      //HEX: 입력받은 hex 패킷에 매핑
    for (int i = 3; i < data_length - 1; i++)        sum += transmit_data[i];           //CheckSum 계산용//checksum = length다음 byte부터 checksum 다 더하고 0xff에서 뺀값
    byte checksum = 255 - (sum % 256);
    transmit_data[data_length - 1] = checksum;                                               // Checksum: 체크섬 마지막에 매핑
    Serial1.write(transmit_data, sizeof(transmit_data));                                 // Xbee3로 전송
    for (int i = 0; i < data_length; i++) {             // 전송된 hex 출력
